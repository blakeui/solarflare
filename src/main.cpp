...
uint256 hashGenesisBlock("0xf57f5277069f2cae3a10653c2ac2d920a7ea8a0db66163e58f48ff1df04b5aa6");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Solarflarecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Solarflarecoin Signed Message:\n";
...
int64 static GetBlockValue(int nHeight, int64 nFees, uint256 prevHash)
{
    ...
    if (nHeight == 1)
    {
        nSubsidy = 3600000 * COIN;  // Development fund

    }

    else if (nHeight < 250) { nSubsidy = 0 * COIN; } // zero-reward to prevent instamine
    else if (nHeight < 5000) { nSubsidy = 50 * COIN; }
    else if (nHeight < 10000) { nSubsidy = 100 * COIN; }
    else if (nHeight < 16000) { nSubsidy = 200 * COIN; }
    else if (nHeight < 24000) { nSubsidy = 350 * COIN; }
    else if (nHeight < 34150) { nSubsidy = 500 * COIN; }
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlock *pblock)
{
    ...
    // Solarflarecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlock *pblock)
{
        ...
        if (fTestNet) {
                if (pindexLast->nHeight+1 >= 10) { DiffMode = 2; }
                                ...
                                if (pindexLast->nHeight+1 >= 9850) { DiffMode = 2; }
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal solarflarecoin miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Solarflarecoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "January 21, 2017: TRUMP IS SWORN IN, VOWING TO REMAKE NATION: THIS AMERICAN CARNAGE STOPS RIGHT HERE // From NYT.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 0 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0412345678900004567890000111222333444555666777888999000000aaaaabbbbbcccccdddddeeeeeff00ff00ff00ff001234567890abcdef0022446688abc89") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1485013053; //Epoch time for mainnet
        ...
        assert(block.hashMerkleRoot == uint256("0xa2042526f1c164be604294e17bee0c22fa876a0004a6d2f45e6bc983b71f801d"));
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc1, 0xa5, 0xdb }; 
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // Solarflarecoind: Reduce the exempted free transactions to 500 bytes (from Bitcoin's 3000 bytes)
